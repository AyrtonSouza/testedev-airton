...
export class FormCorComponent extends PagesBase implements OnInit {
  ...
  camposObrigatorios = {
    ...
    'data_de_cadastro': ['', Validators.compose([Validators.required])],
    'situacao': ['', Validators.compose([Validators.required])]
  ...
  onSubmit(values: Object): void {
    ...
    if (this.form.valid) {
      ...
      } else if (this.cor.data_de_cadastro === '') {
        this.staticModalShow('Atenção', 'Informe o nome cor.');
      } else {
        this.salvarCor(this.cor);
      }
    }
  }
